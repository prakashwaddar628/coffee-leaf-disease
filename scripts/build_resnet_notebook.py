...
'''# 1. Introduction
## ResNet50 Baseline Evaluation

**Goal**: Evaluate whether a deeper residual network can outperform MobileNetV3 while using the exact same experimental setup.

**Research Questions**:
* Does ResNet50 justify its additional computational cost compared to MobileNetV3?
'''))
...
'''))
    ...
    nb.cells.append(nbf.v4.new_markdown_cell("## 6 & 7. Load Model & Transfer Learning (ResNet50)"))
    nb.cells.append(nbf.v4.new_code_cell(
...
print("ResNet50 initialized. Backbone Frozen. Classifier Replaced.")
'''))
...
print("Sprint 6 Execution Completed.")
'''))

    Path('notebooks').mkdir(exist_ok=True)
    with open('notebooks/05_Model_ResNet50.ipynb', 'w') as f:
        nbf.write(nb, f)
